...
const std::string BitcreditGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
BitcreditGUI::BitcreditGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
...
void BitcreditGUI::showDebugWindow()
{
...
void BitcreditGUI::showDebugWindowActivateConsole()
{
...
void BitcreditGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress)
{
    if(!clientModel)
        return;
...
#ifdef ENABLE_WALLET
void BitcreditGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
